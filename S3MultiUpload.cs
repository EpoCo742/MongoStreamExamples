...
using Microsoft.Extensions.Logging; // Ensure to have the proper logging setup

public class MongoToS3Uploader
{
    private readonly IMongoCollection<BsonDocument> _collection;
    private readonly AmazonS3Client _s3Client;
    private readonly ILogger<MongoToS3Uploader> _logger; // Added a logger
    ...
    public async Task StreamDataToS3Async(string bucketName, string objectKey)
    {
        ...
        try
        {
            await foreach (var currentBatch in _collection.FindAsync(filter, options).ToAsyncEnumerable())
            {
        ...
        {
            _logger.LogError(e, "An error occurred during the multipart upload.");
